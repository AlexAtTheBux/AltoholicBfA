...
local function ScanCalendar()
  ...
  local calendarDate = C_Calendar.GetDate()
	local currentMonth, currentYear = calendarDate.month, calendarDate.year
	local thisMonth, thisDay, thisYear = calendarDate.month, calendarDate.monthDay, calendarDate.year
...
local function ScanChallengeModeMaps()
	...
	for _, dungeonID in pairs(maps) do
   	local _, weeklyBestTime, weeklyBestLevel = C_MythicPlus.GetWeeklyBestForMap(dungeonID)
...
local function SetClientServerTimeGap()
	...
	timerHandle = nil
  local calendarDate = C_Calendar.GetDate()
	local ServerMonth, ServerDay, ServerYear = calendarDate.month, calendarDate.monthDay, calendarDate.year
...
function addon:OnEnable()
	...
	local thisMonth, thisYear = C_Calendar.GetDate().month, C_Calendar.GetDate().year
	C_Calendar.SetAbsMonth(thisMonth, thisYear)
	addon:RegisterEvent("CALENDAR_UPDATE_EVENT_LIST", OnCalendarUpdateEventList)
